...
export async function accept_invitation(
  ...
  if (!invitation) {
    return { data: null, error: ERROR_CODE.INVITATION_NOT_FOUND };
  }

  if (invitation.expiresAt && new Date() > new Date(invitation.expiresAt)) {
    return { data: null, error: ERROR_CODE.INVITATION_EXPIRED };
  }
  try {
    if (action === "accept") {
...
export async function create(
  ...
  try {
    ...
    if (existingUser) {
      return { data: null, error: ERROR_CODE.USER_ALREADY_EXISTS };
    }
...
export async function update(
  ...
  try {
    ...
    if (!result) {
      return {
        data: null,
        error: ERROR_CODE.NOT_FOUND ?? ERROR_CODE.FAILED_REQUEST,
      };
    }
