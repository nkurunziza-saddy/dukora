...
export const getProductByIdForStore = async (productId: string) => {
  const result = await productRepo.get_product_by_id_for_store(productId);
  if (result.error) {
    return { data: null, error: result.error };
  }
  return { data: result.data, error: null };
...
export const getCategoriesForStore = async () => {
  const result = await productRepo.get_categories_for_store();
  if (result.error) {
    return { data: null, error: result.error };
  }
  return { data: result.data, error: null };
