...
async function CategoriesContent({ businessId }: { businessId: string }) {
  const { data: categories, error } = await getCategoriesForStore(businessId);
  ...
  if (!categories || categories.length === 0) {
    return (
      <Empty>
        ...
        <EmptyContent>
          <Button render={<Link href={`/store/${businessId}`} />} size={"sm"}>
            {t("backToStore")}
          </Button>
        </EmptyContent>
      </Empty>
    ...
    <div className="pt-10 pb-24 md:pb-32 md:pt-16 lg:pb-40 min-h-[calc(100vh-3rem)]">
      <div className="pgtx">
        ...
        <Suspense fallback={<CategoriesLoading />}>
          <CategoriesContent businessId={(await props.params).businessId} />
        </Suspense>
      </div>
    </div>
  );
